    ...
    public synchronized void add(Aviao aviao) {
        while (isFull()) {
            try {
                synchronized (this) {
                    wait();
                    System.out.println("O aviao " + Thread.currentThread().getName() + " espera para decolar");
                }
            }
    ...
    public synchronized void remove() {
        while (isEmpty()) {
            try {
                synchronized (this) {
                    wait();
                    System.out.println(Thread.currentThread().getName() + " aguarda para decolagem");
                }
            }
            catch (InterruptedException e) {
                e.printStackTrace();
        ...
        itens--;
        System.out.println(Thread.currentThread().getName() + " decolou o aviao");
        synchronized (this) {
            notifyAll();
        }
    }
